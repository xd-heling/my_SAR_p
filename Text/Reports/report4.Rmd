...
Both mean filtering and median filtering can smooth the image and eliminate the noise. Mean filtering adopts linear method to average the pixel value in the whole window range. Mean filter itself has inherent defects. It can not protect the details of the image well, and in the process of denoising, it will destroy the details of the image and make the image become blurred, which leads to the failure to remove the noise points well. Mean filter is better than the impulse noise in filtering low frequency noise. The median filter adopts a nonlinear method, which is very effective in smoothing impulse noise. At the same time, it can protect the sharp edge of the image and select appropriate points to replace the value of the noise point. So it performs better for impulse noise than for low-frequency noise. In addition, the larger the filter is, the fuzzier the result will be, because the filter eliminates the detail of the image over a larger area.